...
- [ ] Фронтенд: `LeadForm.svelte` — чекбокс «Согласие на обработку персональных данных»
- [ ] Бэкенд: `POST /api/leads` — валидация поля `consent: boolean` (обязательно `true`)
- [ ] Бэкенд: возвращать 400 если consent не передан
...
- [ ] `db/init.sql` — суперадмин берёт email из переменной окружения (или оставить `admin@example.com` и обновить `.env`)
- [ ] Тестовый суперадмин — вынести в отдельный seed-файл или сделать условный INSERT
