...
### Этап 8: Уведомления MAX-бот (0.5 дня)

- После `POST /api/leads` → MAX Bot API
...
6. **Домен** — какой домен будет использоваться?
7. **Контакт менеджера** — кому приходят уведомления MAX-бота?
